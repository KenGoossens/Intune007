...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [complianceStatus, devices] = await Promise.all([
      getComplianceStatus(),
    ...
    for (const m of metrics) {
      trends[m] = getDailyTrend(m, days);
    }

    // Compute deltas (compare latest to 7 days ago)
    const computeDelta = (metric: string): { current: number; previous: number; delta: number; direction: string } => {
